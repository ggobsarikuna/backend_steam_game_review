...
public interface MainRepository extends JpaRepository<App_id, Long> {
    ...
    @Query(nativeQuery = true, value="SELECT A.appid, A.name, A.views FROM APP_ID A WHERE A.name LIKE %:str1% DESC VIEWS UNION " +
            "SELECT B.appid, B.name, B.views FROM APP_ID B WHERE B.name LIKE %:str2% DESC VIEWS UNION " +
            "SELECT C.appid, C.name, C.name FROM APP_ID C WHERE C.name DESC VIEWS LIKE %:str3%")
